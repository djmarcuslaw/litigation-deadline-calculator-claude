...
def generate_litigation_deadlines(data):
    ...
    if disco_anchor:
        # Last day to serve interrogatories
        last_interrog = compute_backward_date(disco_anchor, effective_response_days, jurisdiction, holidays)
        deadlines.append({
            "description": f"{matter} — Last Day to Serve Interrogatories",
            "date": last_interrog.isoformat(),
            "rule_basis": f"{rule_prefix} Rule 33 ({response_days}-day response period, must complete before discovery cutoff)",
            "category": "discovery",
            "priority": "high"
        })

        # Last day to serve requests for production
        last_rfp = compute_backward_date(disco_anchor, effective_response_days, jurisdiction, holidays)
        deadlines.append({
            "description": f"{matter} — Last Day to Serve Requests for Production",
            "date": last_rfp.isoformat(),
            "rule_basis": f"{rule_prefix} Rule 34 ({response_days}-day response period, must complete before discovery cutoff)",
            "category": "discovery",
            "priority": "high"
        })

        # Last day to serve requests for admission
        last_rfa = compute_backward_date(disco_anchor, effective_response_days, jurisdiction, holidays)
        deadlines.append({
            "description": f"{matter} — Last Day to Serve Requests for Admission (DANGER: deemed admitted if missed)",
            "date": last_rfa.isoformat(),
            "rule_basis": f"{rule_prefix} Rule 36 ({response_days}-day response period; failure to respond = deemed admitted)",
            "category": "discovery",
            "priority": "critical"
        })
...
def main():
    ...
    result = {
        ...
        "disclaimer": (
            "DISCLAIMER: These deadlines are computed based on general rules and the "
            "scheduling order provided. They should be independently verified by counsel. "
            "Local rules, judge-specific practices, and amended scheduling orders may "
            "affect actual deadlines. This tool is for reference purposes and does not "
